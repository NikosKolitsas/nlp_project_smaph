...
import annotatorstub.cbgeneration.pipeline.EntityExtraction.EntityExtractorWAT;
...
public class EntityGeneratorWAT extends EntityGenerator{

    public static void main(String[] args) throws JSONException {
        EntityGeneratorWAT generator = new EntityGeneratorWAT();
        generator.generate("barack obama goes to the white house to meet putin");
    }

    public EntityGeneratorWAT() {
        this.entityExtractor = new EntityExtractorWAT();
    }
    ...
    public Set<Entity> generate(String query) {
        ...
        ArrayList<ArrayList<WATOutput>> e3Aux = new ArrayList<ArrayList<WATOutput>>();
        int c = 0;
        for (BingWebResult r : top25) {
            e3Aux.add(  ((EntityExtractorWAT) entityExtractor).watAnnotation(r.getDescription(), r.getIndex())   );
            c++;
            if (c % 5 == 0) {
                System.out.print(c + "/25 ... ");
            }
        }
        HashMap<Integer, ArrayList<WATOutput>> e3AuxMap = convertToMap(e3Aux);
        ...
        Set<E3Entity<WATOutput>> e3 = new HashSet<>();
        for (Map.Entry<Integer, ArrayList<WATOutput>> entry : e3AuxMap.entrySet()) {
            e3.add(new E3Entity<WATOutput>(entry.getKey(), entry.getValue(), top25.size(), webTotal, query));
        }
