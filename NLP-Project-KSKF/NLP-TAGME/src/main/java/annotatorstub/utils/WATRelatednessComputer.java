...
public class WATRelatednessComputer implements Serializable {
	...
	private static void queryJsonComm(String anchor) {
		...
		try {
			...
			// TODO: DISABLED // LOG.debug("Querying {}", url);
			JSONObject obj = Utils.httpQueryJson(url);
			...
			for (int i = 0; i < spots.length(); i++) {
				...
				for (int j = 0; j < ranking.length(); j++) {
					...
					instance.cacheComm.put(new Pair<String, Integer>(anchorC, widC), commonnessC); // Replaced anchor with anchorC
				}
			}
		} catch (Exception e) {
			throw new RuntimeException(e);
